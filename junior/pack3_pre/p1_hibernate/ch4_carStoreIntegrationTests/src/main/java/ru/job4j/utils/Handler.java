...
public class Handler {
     ...
     */
    public void handleFormData(HttpServlet servlet, HttpServletRequest req, HashMap<String, String> fields, List<FileItem> files) throws Exception {
        // @see https://commons.apache.org/proper/commons-fileupload/using.html
        // Create a factory for disk-based file items.
        DiskFileItemFactory factory = new DiskFileItemFactory();
        File repository = (File) servlet.getServletContext().getAttribute("javax.servlet.context.tempdir");
    ...
    public void handleFotos(int id, List<FileItem> files) throws Exception {
        ...
        for (FileItem item : files) {
            ...
            if (mimes.contains(mime)) {
                ...
                //boolean im = item.isInMemory();
                //long size = item.getSize();
                //this.logger.error(String.format("File[name: %s, type: %s, IsINMem: %b, size: %d]", fn, ct, im, size));
            }
        }
    }
}