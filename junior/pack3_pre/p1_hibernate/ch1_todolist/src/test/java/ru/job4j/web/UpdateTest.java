...
public class UpdateTest {
    ...
    public void testDoPost() {
        try {
            int id = this.idao.create(new Item(0, "UpdateTest", "testDoPost()", 0L, false));
            ...
            when(req.getParameter("name")).thenReturn("UpdateTestName");
            when(req.getParameter("descr")).thenReturn("UpdateTestDescr");
            when(req.getParameter("created")).thenReturn("2018-04-24 18:39:00");
    ...
    public void testDoPostHasErrors() {
        try {
            int id = this.idao.create(new Item(0, "UpdateTest", "testDoPost()", 0L, false));
            StubServletOutputStream ssos = new StubServletOutputStream();
            String expected = "{\"status\":\"error\",\"errors\":{\"done\":\"isFilled\"}}";
            HttpServletRequest req = mock(HttpServletRequest.class);
            HttpServletResponse resp = mock(HttpServletResponse.class);
            when(req.getParameter("id")).thenReturn(Integer.toString(id));
            when(req.getParameter("name")).thenReturn("UpdateTestName");
            when(req.getParameter("descr")).thenReturn("UpdateTestDescr");
            when(req.getParameter("created")).thenReturn("2018-04-24 18:39:00");
            when(req.getParameter("done")).thenReturn("");
            when(resp.getOutputStream()).thenReturn(ssos);
            this.setAttributeStorage(req);
            servlet.doPost(req, resp);
            String actual = new String(ssos.getByteArray());
    ...
    /**
     * Тестирует public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException.
     * Выброс исключения при попытке получить первый элемент из пустого списка.
     */
    @Test
    public void testDoPostWithExceptionThrowsByMethodGet() {
        try {
            StubServletOutputStream ssos = new StubServletOutputStream();
            String expected = "{\"status\":\"error\",\"errors\":{\"exception\":\"update\"}}";
            HttpServletRequest req = mock(HttpServletRequest.class);
            HttpServletResponse resp = mock(HttpServletResponse.class);
            when(req.getParameter("id")).thenReturn("100500");
            when(req.getParameter("name")).thenReturn("UpdateTestName");
            when(req.getParameter("descr")).thenReturn("UpdateTestDescr");
            when(req.getParameter("created")).thenReturn("2018-04-24 18:39:00");
