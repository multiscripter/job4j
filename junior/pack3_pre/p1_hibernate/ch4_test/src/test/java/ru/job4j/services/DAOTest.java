    ...
    public void beforeTest() {
        try {
            ...
            if (db.equals("HyperSQL")) {
                 ...
                 */
                this.driver = new DBDriver("jdbc:hsqldb:mem:jpack3p1ch4test;get_column_name=false", "SA", "");
            } else if (db.equals("PostgreSQL")) {
                this.driver = new DBDriver("jdbc:postgresql://localhost:5432/jpack3p1ch4test", "postgres",
 "postgresrootpass");
            }
            ...
            this.path = String.format("%s../../src/test/resources/junior.pack3.p1.ch4.test.%s.sql", this.path, db);
    ...
    public void testCreateWithTypeCar() {
        try {
            ...
            int id = this.dao.create(expected);
            expected.setId(id);
            String query = String.format("select cars.id as \"car_id\", cars.name as \"car_name\" from cars where cars.id = %d", expected.getId());
            List<HashMap<String, String>> result = this.driver.select(query);
            Car actual = new Car();
    ...
    public void testReadWithTypeCar() {
        try {
            ...
            String query = "select cars.id as \"car_id\", cars.name as \"car_name\" from cars order by cars.name";
            List<HashMap<String, String>> result = this.driver.select(query);
            for (HashMap<String, String> item : result) {
                Car car = new Car();
