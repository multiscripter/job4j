...
public class DBDriverTest {
    ...
    public static void beforeAllTests() {
        try {
            if (db.equals("H2")) {
                ...
                url = "jdbc:postgresql://localhost:5432/jpack3p1ch1task2";
                user = "postgres";
    ...
    public void beforeEachTest() {
        try {
            driver.executeSqlScript(String.format("%s../../src/test/resources/junior.pack3.p1.ch1.task2.%s.sql", path, db));
        } catch (Exception ex) {
            logger.error("ERROR", ex);
            ex.printStackTrace();
        }
    ...
    public void testDelete() {
        try {
            int affected = driver.delete("delete from items");
    ...
    public void testExecuteSql() {
        try {
            driver.executeSql("select * from items");
    ...
    public void testExecuteSqlScriptThrowsSQLException() throws SQLException {
        try {
            driver.executeSqlScript(path + "../../src/test/resources/junior.pack3.p1.ch1.task2.test.sql");
    ...
    public void testSelect() {
        try {
            LinkedList<HashMap<String, String>> result = driver.select("select * from items");
    ...
    public void testUpdate() {
        try {
            int affected = driver.update("update items set done = true where id = 1");
            assertEquals(1, affected);
        } catch (Exception ex) {
            logger.error("ERROR", ex);
            ex.printStackTrace();
